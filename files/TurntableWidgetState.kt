...
internal object TurntableWidgetState {
    ...
    /** Live FFT/magnitude points (0..800 each). Updated ~60 Hz while playing. */
    val wavePoints = AtomicReference<List<Float>>(emptyList())

    fun updateWave(points: List<Float>) {
        wavePoints.set(points)
    }

    fun clearWave() {
        wavePoints.set(emptyList())
    }
}
