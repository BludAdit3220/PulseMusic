package com.pulsemusic.paxsenix

import android.content.Context
import com.pulsemusic.music.betterlyrics.TTMLParser
import com.pulsemusic.paxsenix.models.LyricsResponse
import com.pulsemusic.paxsenix.models.SearchResponse
import com.pulsemusic.paxsenix.models.SearchResult
...
object Paxsenix {
        ...
        synchronized(this) {
            ...
            client = HttpClient(CIO) {
                ...
                defaultRequest {
                    ...
                    header("User-Agent", "PulseMusic/$appVersion")
                }
    ...
    private suspend fun search(query: String): List<SearchResult> = runCatching {
        ...
        val responseText = httpClient.get("/apple-music/search") {
            parameter("q", query)
        }.body<String>()
        
        // Handle cases where API returns an error object instead of a list
        if (responseText.trim().startsWith("{")) {
            Timber.w("Received JSON object instead of array from Paxsenix search: $responseText")
    ...
    suspend fun getLyrics(
        ...
        if (lrc.isNotEmpty()) {
            if (!hasWordTimings) {
                Timber.d("Paxsenix only has plain lyrics for this track")
            }
            return Result.success(lrc)
        }
        Timber.w("No lyrics found from Paxsenix, letting other providers handle it")
